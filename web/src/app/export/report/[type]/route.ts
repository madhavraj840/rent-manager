...
import { RANGED, REPORTS, buildReport, csvResponse, isDay, slug, toCsv, type ReportType } from "@/server/reports";
...
export async function GET(req: NextRequest, ctx: RouteContext<"/export/report/[type]">) {
  ...
  const range = RANGED.includes(type as ReportType) ? `${from}_${to}` : p.ctx.today;
